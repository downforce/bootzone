...
<body>
  <div class="container-fluid">
    <div class="row-fluid">
      <div id="headerButtons">
          <input type="button" class="btn btn-small btn-primary" value="<?= $SLANG['More'] ?>" onclick="expandLog()"/>
          <input type="button" class="btn btn-small btn-primary" value="<?= $SLANG['Clear'] ?>" onclick="clearLog()"/>
          <input type="button" class="btn btn-small btn-primary" value="<?= $SLANG['Refresh'] ?>" onclick="refreshLog()"/>
          <input type="button" class="btn btn-small btn-primary" value="<?= $SLANG['Export'] ?>" onclick="exportLog()"/>
          <input type="button" class="btn btn-small btn-inverse" value="<?= $SLANG['Close'] ?>" onclick="closeWindow()"/>
      </div>
      <h3 class="floating"><?= $SLANG['SystemLog'] ?></h3>
      <div class="row-fluid">
    ...
    <div class="row-fluid">
      ...
      <form name="logForm" method="post" action="<?= $_SERVER['PHP_SELF'] ?>">
        ...
        <table id="logTable" class="table table-condensed table-hover" cellspacing="0">
          <thead>
            <tr>
  ...
  <div id="exportLog" class="overlay row-fluid">
    ...
    <div class="overlayBody">
      <div class="overlayContent">
        <form id="exportForm" action="" method="post">
          ...
          <input type="button" class="btn btn-small btn-primary" id="exportButton" value="<?= $SLANG['Export'] ?>" onclick="exportRequest()"/>
          <input type="button" class="btn btn-small btn-inverse" value="<?= $SLANG['Cancel'] ?>" class="overlayCloser"/>
        </form>
      </div>
    </div>
  </div>
