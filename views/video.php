...
?>
      <form name="contentForm" id="contentForm" method="post" action="<?= $_SERVER['PHP_SELF'] ?>">
        ...
        <input type="button" class="btn btn-small btn-primary" value="<?= $SLANG['GenerateVideo'] ?>" onclick="generateVideo( this.form );"<?php if ( !ZM_OPT_FFMPEG ) { ?> disabled="disabled"<?php } ?>/>
      </form>
<?php
    if ( isset($_REQUEST['generated']) )
    {
?>
      <h3 id="videoProgress" class="<?= $_REQUEST['generated']?'infoText':'errorText' ?>"><span id="videoProgressText"><?= $_REQUEST['generated']?$SLANG['VideoGenSucceeded']:$SLANG['VideoGenFailed'] ?></span><span id="videoProgressTicker"></span></h3>
<?php
    }
    else
    {
?>
      <h3 id="videoProgress" class="hidden warnText"><span id="videoProgressText"><?= $SLANG['GeneratingVideo'] ?></span><span id="videoProgressTicker"></span></h3>
<?php
    }
?>
      <h3 id="videoFilesHeader"><?= $SLANG['VideoGenFiles'] ?></h3>
<?php
...
<?php
    ...
    {
?>
      <table id="videoTable" class="table table-condensed table-hover" cellspacing="0">
        <thead>
          <tr>
