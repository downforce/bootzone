...
<body>
  <div class="container-fluid">
    <div class="row-fluid">
      <h3><?= validHtmlStr($monitor['Name']) ?> - <?= $SLANG['Settings'] ?></h3>
    </div>
    <div id="content" class="row-fluid">
      <form name="contentForm" id="contentForm" method="post" action="<?= $_SERVER['PHP_SELF'] ?>">
        ...
        <div id="contentButtons">
          <input type="submit" class="btn btn-small btn-success" value="<?= $SLANG['Save'] ?>"<?php if ( !canView( 'Control' ) ) { ?> disabled="disabled"<?php } ?>/><input type="button" class="btn btn-small btn-inverse" value="<?= $SLANG['Close'] ?>" onclick="closeWindow()"/>
        </div>
      </form>
    </div>
  </div>
