  ...
  <div class="container-fluid">
    <div class="navbar">
      <div class="navbar-inner">
        <a class="brand" href="#"><?= $SLANG['Options'] ?></a>
...
<?php
    ...
    {
?>
          <li class=""><a href="?view=<?= $view ?>&amp;tab=<?= $name ?>"><?= $value ?></a></li>
<?php
    }
}
?>
...
?>
	...
	<form name="optionsForm" method="get" action="<?= $_SERVER['PHP_SELF'] ?>">
		...
		<table class="table table-bordered table-striped optionTable" cellspacing="0">
        ...
        <div class="row-fluid">
          <input type="submit" class="btn btn-small btn-success" value="<?= $SLANG['Save'] ?>"<?= $canEdit?'':' disabled="disabled"' ?>/>
...
?>
            <tr>
              ...
              <td class="colMark"><label class="checkbox"><input type="checkbox" name="markUids[]" value="<?= $row['Id'] ?>" onclick="configureDeleteButton( this );"<?php if ( !$canEdit ) { ?> disabled="disabled"<?php } ?>/></label></td>
            </tr>
<?php
    }
?>
        ...
        <div class="row-fluid">
          <input type="button" class="btn btn-small btn-primary" value="<?= $SLANG['AddNewUser'] ?>" onclick="createPopup( '?view=user&amp;uid=0', 'zmUser', 'user' );"<?php if ( !canEdit( 'System' ) ) { ?> disabled="disabled"<?php } ?>/><input type="submit" class="btn btn-small btn-danger" name="deleteBtn" class="btn btn-small btn-danger" value="<?= $SLANG['Delete'] ?>" disabled="disabled"/><input type="button" class="btn btn-small btn-inverse" value="<?= $SLANG['Cancel'] ?>" onclick="closeWindow();"/>
        </div>
      </form>
<?php
}
...
?>
      <form name="optionsForm" method="post" action="<?= $_SERVER['PHP_SELF'] ?>">
        ...
        <table id="contentTable" class="table table-striped table-bordered table-hover table-condensed optionTable" cellspacing="0">
          <thead>
            <tr>
              <th><?= $SLANG['Name'] ?></th>
...
?>
            <tr>
              <td><?= $shortName ?></td>
              <td><?= validHtmlStr($optionPromptText) ?>&nbsp;(<?= makePopupLink( '?view=optionhelp&amp;option='.$name, 'zmOptionHelp', 'optionhelp', '<i class="icon-question-sign"></i>' ) ?>)</td>
<?php   
        if ( $value['Type'] == "boolean" )
        {
?>
              <td><label class="checkbox"><input type="checkbox" id="<?= $name ?>" name="newConfig[<?= $name ?>]" value="1"<?php if ( $value['Value'] ) { ?> checked="checked"<?php } ?><?= $canEdit?'':' disabled="disabled"' ?>/></label></td>
<?php
        }
...
?>
                <span><label class="radio inline"><input type="radio" id="<?= $name.'_'.preg_replace( '/[^a-zA-Z0-9]/', '', $optionValue ) ?>" name="newConfig[<?= $name ?>]" value="<?= $optionValue ?>"<?php if ( $value['Value'] == $optionValue ) { ?> checked="checked"<?php } ?><?= $canEdit?'':' disabled="disabled"' ?>/>&nbsp;<?= htmlspecialchars($optionLabel) ?></label></span>
<?php
                }
            }
?>
...
?>
        ...
        <div class="row-fluid">
          <input type="submit" class="btn btn-small btn-success" value="<?= $SLANG['Save'] ?>"<?= $canEdit?'':' disabled="disabled"' ?>/> <input type="button" class="btn btn-small btn-inverse" value="<?= $SLANG['Cancel'] ?>" onclick="closeWindow();"/>
        </div>
      </form>
<?php
}
