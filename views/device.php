...
<body>
  <div class="container-fluid">
    <div class="row-fluid">
      <h3><?= $SLANG['Device']." - ".validHtmlStr($newDevice['Name']) ?></h3>
    </div>
    <div id="content" class="row-fluid">
      <form name="contentForm" method="get" action="<?= $_SERVER['PHP_SELF'] ?>">
        ...
        <div id="contentButtons">
          <input type="submit" class="btn btn-small btn-primary" value="<?= $SLANG['Save'] ?>"<?php if ( !canEdit( 'Devices' ) ) { ?> disabled="disabled"<?php } ?>/><input type="button" class="btn btn-small btn-inverse" value="<?= $SLANG['Cancel'] ?>" onclick="closeWindow()"/>
        </div>
      </form>
    </div>
  </div>
